...
FILE_NAME = f'player_info_{START_SEASON}_{END_SEASON}'
FILE_FORMAT = '.parquet'
...
def ingest_player_info_hist():
    
    @task()
    def get_player_info(seasons,teams):
        """
    ...
    player_info_json = get_player_info(seasons_list,teams_json)
    ...
    teams_json >> seasons_list >> player_info_json >> player_info_pq >> player_info_upload >> player_info_remove_local >> \
    player_info_create_external_table >> player_info_insert_external_table >> player_info_delete_external_table

ingest_player_info_hist()
